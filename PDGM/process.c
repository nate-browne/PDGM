...
ParticipantProc_t * start_participant_process(const char *exe_name) {
  int pid;
  ...
  if((pid = fork()) == 0) {
    ...
    if(to_prog[0] >= 2) close(to_prog[0]); // if this is 0 or 1, dupe2 closed the original
